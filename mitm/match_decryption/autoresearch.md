...
- Level-up refinement: `1086` type `0x42` still looks useful as a fallback level signal for players who otherwise show zero level-ups, but its stat-followup window is slightly later / wider than `0x3e`. A `0.35s / 80-message` lookahead recovered a plausible first level-up for `Thorn` and one extra level milestone for `Fasoot` without the explosion seen from broader window changes.
