...
- Kill-attribution correction: reward-window fallbacks were sometimes crediting kills to the dead player's teammate because post-death gold/xp pulses include allied recipients. Restricting candidate killers to the opposing team removed those friendly-fire artifacts. After that fix, widening the opponent-only reward fallback window from `0.5s` to `1.0s` recovered additional plausible kill attributions (mostly clean unique opponent reward recipients) without reintroducing same-team killers, raising kill coverage again.
- Conservative ARAL level fallback: after the main temporal level detector runs, some long ARAL players still remained stuck at base level 4 despite very high XP totals. A second-pass fallback that only applies to zero-level players with `xp_counter >= 7000` and a nearby `0x42` pulse followed by exactly one stat-type change recovered five clearly undercounted players (`Temporary skyandsoul`, `Temporary Natora`, `hacked`, `TRK`, `atta`) without broadening the earlier level heuristics.
