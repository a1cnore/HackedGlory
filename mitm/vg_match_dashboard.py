...
class Player:
    ...
    hero: str = ""
    # Tracked stats (from opcode 1086 type 0x3e / 0x4d combined counters)
    gold_counter: int = 0  # major*256+minor from type 0x4d
    xp_counter: int = 0  # major*256+minor from type 0x3e
...
def decode_snapshot(payload: bytes, state: MatchState):
    ...
    for slot in range(6):
        ...
        if team in (1, 2):
            p.team = team
        if gold_snap != 0xFFFF:
            p.gold_counter = max(p.gold_counter, gold_snap)
...
def decode_entity_prop(payload: bytes, state: MatchState):
    ...
    Type 0x3e: XP-like counter (u32 major at [9:13], u8 minor at [13])
    Type 0x4d: Gold-like counter
    """
    if len(payload) < 14:
        return
    entity_id = struct.unpack(">H", payload[2:4])[0]
