...
| Tool | Purpose |
|------|---------|
| `mitm/vg_dashboard_server.py` | HTTP server with JSON API for real-time match decoding |
| `mitm/vg_dashboard.html` | Browser-based live dashboard with 2D map, scoreboard, events |
| `mitm/vg_match_dashboard.py` | CLI match analysis dashboard |
...
| `reports/decoded_matches/*.jsonl` | Pre-decoded match data (all messages as JSON) |
