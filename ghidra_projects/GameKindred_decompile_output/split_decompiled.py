...
INPUT_C = "/Users/marvinkleinpass/ghidra_projects/GameKindred_decompile_output/GameKindred.c"
INPUT_H = "/Users/marvinkleinpass/ghidra_projects/GameKindred_decompile_output/GameKindred.h"
OUTPUT_DIR = "/Users/marvinkleinpass/ghidra_projects/GameKindred_decompile_output/structured"
...
def main():
    print("=" * 60)
    print("GameKindred Decompilation Splitter v5")
    print("=" * 60)

    size_mb = os.path.getsize(INPUT_C) / (1024 * 1024)
    print(f"Input: GameKindred.c ({size_mb:.1f} MB)")
    ...
    shutil.copy2(INPUT_H, os.path.join(OUTPUT_DIR, "headers", "GameKindred.h"))
    print(f"  headers/GameKindred.h — {os.path.getsize(INPUT_H) / (1024*1024):.1f} MB")
    os.symlink("headers/GameKindred.h", os.path.join(OUTPUT_DIR, "GameKindred.h"))
        ...
        for line in f:
            ...
            if not in_func:
                if opens > 0 and depth == 0:
                    ...
                    # Look backward in func_buf for signature lines
                    in_func = True
                    ...
                    # But only if we haven't found any functions yet
                    if func_count == 0 and len(func_buf) > 100:
                        # Move all but last 10 lines to preamble
